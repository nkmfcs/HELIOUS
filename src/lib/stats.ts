import { COLORS, SELL_PRICE, type CashKind, type CashTxn, type Color, type Costs, type Order, type WarehouseState } from "./types";
import { monthKey, stockTotal } from "./format";
...
export function dayReport(state: WarehouseState, date: string): DayReport {
  const ev = state.events.filter((e) => e.date === date);
  return {
    date,
    incomingPairs: ev.filter((e) => e.type === "incoming").reduce((s, e) => s + e.pairs, 0),
    orderCount: ev.filter((e) => e.type === "order").length,
    shippedPairs: ev.filter((e) => e.type === "order").reduce((s, e) => s + e.pairs, 0),
    billed: ev.filter((e) => e.type === "order").reduce((s, e) => s + e.sum, 0),
    paid: ev.filter((e) => e.type === "payment").reduce((s, e) => s + e.sum, 0),
...
export function monthDays(state: WarehouseState, ym: string): DayReport[] {
  const dates = new Set(state.events.filter((e) => monthKey(e.date) === ym).map((e) => e.date));
...
export function availableMonths(state: WarehouseState): string[] {
  const set = new Set(state.events.map((e) => monthKey(e.date)));
...
export function cashTotals(state: Pick<WarehouseState, "cash">) {
  ...
  const sum = (kind: CashKind) => cash.filter((t) => t.kind === kind).reduce((s, t) => s + t.amount, 0);
  return {
    income: sum("income"),
    ...
    other: sum("other"),
    spent: sum("material") + sum("worker") + sum("withdraw") + sum("other"),
  };
}
